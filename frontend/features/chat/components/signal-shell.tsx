      ...
      <AnimatePresence>
        {(isOffline || socketBanner) && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -40 }}
            className="absolute left-1/2 top-4 z-50 flex -translate-x-1/2 items-center rounded-full bg-amber-500/90 px-4 py-1.5 text-sm font-medium text-amber-50 shadow-lg backdrop-blur-md"
          >
            {isOffline ? "Waiting for network..." : socketBanner}
          </motion.div>
        )}
      </AnimatePresence>
