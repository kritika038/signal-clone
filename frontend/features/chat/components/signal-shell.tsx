...
export function SignalShell() {
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden bg-white dark:bg-signal-dark-bg text-neutral-900 dark:text-white">
      <AnimatePresence>
        {(isOffline || socketBanner || featureNotice) && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -40 }}
            className="absolute left-1/2 top-4 z-50 flex -translate-x-1/2 items-center rounded-full bg-signal-blue-500/90 px-4 py-1.5 text-sm font-medium text-white shadow-lg backdrop-blur-md"
          >
      ...
      <aside className={`${isSidebarOpen ? "flex" : "hidden"} w-80 flex-col border-r border-neutral-200 dark:border-signal-dark-bubble bg-neutral-50 dark:bg-signal-dark-sidebar md:flex relative`}>
        ...
        <div className="absolute bottom-6 right-6 z-40 flex flex-col items-end">
          ...
          <button
            ...
            className="flex h-14 w-14 items-center justify-center rounded-2xl bg-signal-blue-500 text-white shadow-lg shadow-blue-900/20 transition-all hover:bg-signal-blue-500 hover:scale-105 active:scale-95"
      ...
      {/* Main Chat Area */}
      <main className="flex min-w-0 flex-1 flex-col bg-white dark:bg-signal-dark-bg">
        {activeConversationId ? (
          <>
            {/* Chat Header */}
                  ...
                  {queuedAttachments.length > 0 && (
                    <div className="flex flex-wrap gap-2 px-12 mb-2">
                      {queuedAttachments.map((attachment) => (
                        <div key={attachment.id} className="flex items-center gap-2 rounded-lg bg-white dark:bg-signal-dark-bubble p-2 shadow-sm border border-neutral-200 dark:border-[#2B2B2B]">
                          <div className="flex h-8 w-8 items-center justify-center rounded-md bg-blue-50 dark:bg-blue-900/20 text-signal-blue-500">
                          ...
                          <div className="flex flex-col">
                            <span className="text-xs font-medium text-neutral-900 dark:text-white truncate max-w-[120px]">{attachment.name}</span>
                            <span className="text-[10px] text-neutral-500 dark:text-signal-dark-textSecondary">{attachment.sizeLabel}</span>
                          </div>
